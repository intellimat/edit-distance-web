function colorCell(row, col, color) {
  ...
  } else console.log(`cell[${i}][${j}] is equal to `, cell);
}
...
function drawMatrix(matrix, v_string, h_string) {
  ...
  for (let i = 0; i < matrix.length; i++) {
    ...
    } else verticalLetter_th.innerHTML = " ";
...
function colorBackTrack(D, r, c) {
  window.setTimeout(() => {
    ...
    if (r === 0) topForbidden = true;
    if (c === 0) leftForbidden = true;

    if (topForbidden && leftForbidden) return;

    if (topForbidden) colorBackTrack(D, r, c - 1);
    else if (leftForbidden) colorBackTrack(D, r - 1, c);
    else {
      ...
      if (diagonal === min) colorBackTrack(D, r - 1, c - 1);
      else if (left === min) colorBackTrack(D, r, c - 1);
      else colorBackTrack(D, r - 1, c);
    }
  }, 200);
}
...
function getInitialMatrix(s1, s2) {
  ...
  for (let i = 1; i < numberOfRows; i++) {
    row = [i];
    for (let k = 1; k < numberOfCols; k++) row.push("");
    D.push(row);
  }
...
function getMin(...args) {
  min = args[0];
  for (let value of args) if (value < min) min = value;
  return min;
}
...
function getEditDistanceMatrix(s1, s2) {
  ...
  for (j = 1; j < numberOfCols; j++)
    for (i = 1; i < numberOfRows; i++) {
      ...
      if (s1[i - 1] === s2[j - 1]) D[i][j] = getMin(insertion, deletion, match);
      else D[i][j] = getMin(insertion, deletion, mismatch);
    }
  return D;
}
